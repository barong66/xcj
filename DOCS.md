...
- Аналитика: banner_impression (показ) и banner_click (клик) в ClickHouse

**Размеры баннеров (настраиваемые):**
...
**Управление:**
- Вкладка "Promo" на странице аккаунта — просмотр баннеров, ручная генерация
- Раздел "Promo" в sidebar — все баннеры по всем платным аккаунтам, добавление размеров
...
- **Banner Worker** — генерирует баннеры из тумб видео (crop, resize, JPEG) для платных аккаунтов, загружает в R2
...
- **banner_sizes** — глобальные размеры баннеров (300x250, 728x90, etc.)
